using System;
using System.Text;
namespace Lab_8
{
    public class Blue_3 : Blue
    {
        private (char, double)[] _output;

        public (char, double)[] Output
        {
            get
            {
                if (_output == null) return null;
                (char, double)[] copy = new (char, double)[_output.Length];
                Array.Copy(_output, copy, copy.Length);
                return copy;
            }
        }

        public Blue_3(string input) : base(input)
        {
            _output = null;
        }

        public override void Review()
        {
            if (string.IsNullOrEmpty(Input))
            {
                _output = null;
                return;
            }

            char[] razdelialki = { ' ', '.', '!', '?', ',', ':', '\"', ';', '–', '(', ')', '[', ']', '{', '}', '/' };
            string[] wordiksi = Input.Split(razdelialki, StringSplitOptions.RemoveEmptyEntries);

            int[] cntOfCharov = new int[char.MaxValue];
            int wordikiCnt = 0;

            foreach (string word in wordiksi)
            {
                if (word.Length == 0) continue;

                char firstChar = char.ToLower(word[0]);
                if (char.IsLetter(firstChar))
                {
                    cntOfCharov[firstChar]++;
                    wordikiCnt++;
                }
            }

            int uniqueLettersCnt = 0;
            for (int i = 0; i < cntOfCharov.Length; i++)
            {
                if (cntOfCharov[i] > 0) uniqueLettersCnt++;
            }

            _output = new (char, double)[uniqueLettersCnt];
            int index = 0;

            for (int i = 0; i < cntOfCharov.Length; i++)
            {
                if (cntOfCharov[i] > 0)
                {
                    double result = Math.Round(cntOfCharov[i] * 100.0 / wordikiCnt, 4);
                    _output[index++] = ((char)i, result);
                }
            }

            Array.Sort(_output, (x, y) =>
            {
                int cmp = y.Item2.CompareTo(x.Item2);
                return cmp != 0 ? cmp : x.Item1.CompareTo(y.Item1);
            });
        }

        public override string ToString()
        {
            if (_output == null || _output.Length == 0) return "";

            var str = new StringBuilder();
            for (int i = 0; i < _output.Length; i++)
            {
                var x = _output[i];
                str.Append($"{x.Item1} - {x.Item2:F4}");
                if (i < _output.Length - 1)
                {
                    str.AppendLine();
                }
            }
            return str.ToString();
        }
    }
}